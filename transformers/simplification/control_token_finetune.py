...
from control_token_lightning_base import add_generic_args, generic_train, PrefixTransformer  # noqa
...
class PrefixSummarizationModule(PrefixTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        if self.hparams.new_tokens:
            ...
            make_new_embeddings_learnable(self.seq2seq_model,self.new_token_len)
    ...
    def training_step(self, batch, batch_idx) -> Dict:
        if batch_idx == 0:
            ...
            print('Trainable',self.seq2seq_model.model.encoder.embed_tokens.trainable_weight)
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        ...
        cmd = f"""easse evaluate -t custom \
--refs_sents_paths "/content/gdrive/MyDrive/turkcorpus/test.truecase.detok.simp.0","/content/gdrive/MyDrive/turkcorpus/test.truecase.detok.simp.1","/content/gdrive/MyDrive/turkcorpus/test.truecase.detok.simp.3","/content/gdrive/MyDrive/turkcorpus/test.truecase.detok.simp.4" \
--orig_sents_path "/content/gdrive/MyDrive/turkcorpus/test.truecase.detok.orig" \
--sys_sents_path "{self.val_output}" \
-m 'bleu,sari,fkgl' """


        #bleu = float(re.search("'bleu': ([^,]+)", output).group(1))
        print(output)
        metric_val = (
            sari
        )
    ...
    -m 'bleu,sari,fkgl' """
        ...
        sari_turk = float(re.search("'sari': ([^,]+)", output).group(1))
        print('SARI',sari)
        print('SARI_TURK',sari_turk)
        print('VAL_LOSS',loss)
