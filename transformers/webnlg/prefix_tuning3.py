...
class PrefixTuning(PretrainedBartModel):
    ...
    def __init__(self, config, preseqlen=5):
        ...
        self.categories = ['sources', 'cats']
        self.new_token_len = [6, 17]
        ...
        if self.unseen is True:
            ...
            self.M_Prefixes[f'wte_cats'].weight[-2:] =torch.zeros(self.M_Prefixes[f'wte_cats'].weight[-2:].shape).to(self.device)
            self.M_Prefixes[f'wte_enc_cats'].weight[-2:] =torch.zeros(self.M_Prefixes[f'wte_cats'].weight[-2:].shape).to(self.device)
            self.M_Prefixes[f'wte_2_cats'].weight[-2:] =torch.zeros(self.M_Prefixes[f'wte_cats'].weight[-2:].shape).to(self.device)
    ...
    def get_prompt_multiple_prefix(self, conditional_info, bsz=None, sample_size=1):
        ...
        for category_idx, category in enumerate(self.categories):
            self.seed_multiple = torch.arange(self.m_prefix_len * self.new_token_len[category_idx]).long().unsqueeze(
            ...
            past_key_values_multiple = self.control_trans(temp_control)  # bsz 3*seqlen ,
            ...
            past_key_values_multiple = torch.stack(
                [past_key_values_multiple[i, j[0]:j[1], :] for i, j in enumerate(cond)])
