...
class PrefixSummarizationModule(PrefixTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        self.val_dir = self.hparams.val_dir
        self.val_output = Path(self.val_dir) / "val_output"
        self.val_input = Path(self.val_dir) / "val.source"
        self.ref_m2_val = Path(self.val_dir) / "val_ref.m2"
        self.out_m2 = Path(self.val_dir) / "out.m2"
        os.system(f'rm {self.val_output}')
        os.system(f'rm {self.out_m2}')
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        outputs = self(src_ids, attention_mask=src_mask, decoder_input_ids=decoder_input_ids, use_cache=False,
                       use_prefix=True,CEFR =batch['CEFR'])
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        ...
        cmd = f"""errant_parallel -orig {self.val_input} -cor {self.val_output} -out {self.out_m2} -tok"""
        subprocess.run(cmd, shell = True,check=True)
        cmd = f"""errant_compare -hyp {self.out_m2} -ref {self.ref_m2_val}"""


        output = subprocess.check_output(cmd, shell = True).decode("utf-8")
        f_measure = float('0.'+re.search(r'\d+', output[::-1]).group()[::-1])
    ...
    def _generative_step(self, batch: dict) -> dict:
        ...
        prefix_prompt = self.model.get_prompt(bsz=bsz, sample_size=self.eval_beams,CEFR=batch['CEFR'])
        # print(prefix_prompt)
        ...
        with open(self.val_output, "a") as f:
            for pred in preds:
                print(pred, file = f)
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        #     dataset,
        #     collate_fn = dataset.collate_fn,
        #     batch_sampler = CustomBatchSampler_2(),
        #     num_workers = self.num_workers,
        # ) # need to use reset trainloader
    ...
    def val_dataloader(self) -> DataLoader:
        ...
        if self.hparams.data_dir == "/content/gdrive/MyDrive/val_no_N":
            print('AaAAaaaaaa')
        ...
        dataloader = DataLoader(
            dataset,
            collate_fn = dataset.collate_fn,
            batch_sampler = CustomBatchSampler_val(),
            num_workers = self.num_workers,
            shuffle = False
        )
        return dataloader
