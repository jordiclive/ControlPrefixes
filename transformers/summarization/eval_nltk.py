...
from lightning_base_multi import add_generic_args, generic_train, PrefixTransformer  # noqa
...
class PrefixSummarizationModule(PrefixTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        self.decode_output = Path(self.val_dir) / "tar_output"
        os.system(f'rm {self.val_output}')
        os.system(f'rm {self.decode_output}')
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        outputs = self(src_ids, attention_mask = src_mask, decoder_input_ids = decoder_input_ids, use_cache = False,
                       use_prefix = True, conditional_info = {'cats':batch['cats'],'sport':batch['sport']})
    ...
    def _generative_step(self, batch: dict) -> dict:
        ...
        prefix_prompt = self.model.get_prompt(bsz = bsz, sample_size = self.eval_beams,
                                              conditional_info = {'cats':batch['cats'],'sport':batch['sport']})
        # print(prefix_prompt)
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--logger_name", type = str, choices = ["default", "wandb", "wandb_shared"],
                            default = "default")
        ...
        parser.add_argument("--eval_max_gen_length", type = int, default = 60,
...
def eval(args, model=None):
    ...
    with torch.no_grad():
        model.eval()
        model = model.cuda()
        print("eval_beams",model.eval_beams)
        print("eval_min_length",model.eval_min_length)
        print("max_length",model.eval_max_length)
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            with open(model.val_output, "a") as f:
                for pred in decoded_preds:
                    print(pred, file = f)
            with open(model.decode_output, "a") as f:
                for tar in decoded_labels:
                    print(tar, file = f)
