...
class PrefixTuning(PretrainedBartModel):
    ...
    def __init__(self, config, preseqlen=5):
        ...
        self.wte = nn.Embedding(self.preseqlen, self.first)
        self.control_trans = nn.Sequential(
            nn.Linear(self.first, self.mid_dim),
            nn.Tanh(),
        ...
        if self.use_encoder_prefix:
            self.wte_enc = nn.Embedding(self.preseqlen, self.first)
        ...
        if self.use_cross_prefix:
            self.wte2 = nn.Embedding(self.preseqlen, self.first)
