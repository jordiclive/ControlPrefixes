...
def run_eval_tester(model):
    ...
    task = "translation_en_to_de" if model == T5_TINY else "GEC"
    testargs = f"""
        run_eval_search.py
        {model}
        {input_file_name}
...
# testing with 2 models to validate: 1. translation (t5) 2. GEC (mbart)
@slow
...
def test_finetune(model):
    args_d: dict = CHEAP_ARGS.copy()
    task = "translation" if model in [MBART_TINY, MARIAN_TINY, FSMT_TINY] else "GEC"
...
def test_finetune_lr_schedulers():
    ...
    # emulate finetune_summarization.py
