...
class ControlPrefixes(PretrainedBartModel):
    ...
    def __init__(self, config, preseqlen=5):
        ...
        input_length = (
            self.preseqlen
            + (self.m_prefix_len[0] * self.new_token_len[0])
            + (self.m_prefix_len[1] * self.new_token_len[1])
        )
    ...
    def get_prompt_multiple_prefix(self, conditional_info, bsz=None, sample_size=1):
        ...
        idxmap = {
            i: ((i) * self.m_prefix_len[0], ((i + 1) * self.m_prefix_len[0]))
            for i in range(self.new_token_len[0])
        }

        cond = list(map(idxmap.get, conditional_info["sport"].tolist()))

        idxmap2 = {
            i: ((i) * self.m_prefix_len[1], ((i + 1) * self.m_prefix_len[1]))
        ...
        past_key_values_multiple = torch.stack(
            [
                past_key_values[i, self.preseqlen + j[0] : self.preseqlen + j[1], :]
                for i, j in enumerate(cond)
            ]
        )
        past_key_values_multiple_2 = torch.stack(
            [
                past_key_values[i, start_idx + j[0] : start_idx + j[1], :]
                for i, j in enumerate(cond2)
            ]
        )

        past_key_values = torch.cat(
            [
                past_key_values_multiple,
                past_key_values_multiple_2,
                past_key_values[:, : self.preseqlen, :],
            ],
            dim=1,
        )
        ...
        past_key_values_multiple = torch.stack(
            [
                past_key_values2[i, self.preseqlen + j[0] : self.preseqlen + j[1], :]
                for i, j in enumerate(cond)
            ]
        )
        past_key_values_multiple_2 = torch.stack(
            [
                past_key_values2[i, start_idx + j[0] : start_idx + j[1], :]
                for i, j in enumerate(cond2)
            ]
        )
        ...
        past_key_values2 = torch.cat(
            [
                past_key_values_multiple,
                past_key_values_multiple_2,
                past_key_values2[:, : self.preseqlen, :],
            ],
            dim=1,
        )
        ...
        past_key_values_multiple = torch.stack(
            [
                past_key_values_enc[i, self.preseqlen + j[0] : self.preseqlen + j[1], :]
                for i, j in enumerate(cond)
            ]
        )
        past_key_values_multiple_2 = torch.stack(
            [
                past_key_values_enc[i, start_idx + j[0] : start_idx + j[1], :]
                for i, j in enumerate(cond2)
            ]
        )
        ...
        past_key_values_enc = torch.cat(
            [
                past_key_values_multiple,
                past_key_values_multiple_2,
                past_key_values_enc[:, : self.preseqlen, :],
            ],
            dim=1,
        )
        ...
        for i, key_val in enumerate(past_key_values):
            temp_dict = {
                "self": {
                    "prev_key": key_val[
                        0
                    ].contiguous(),  # [16, 12, 200, 64] # for each head there are 12 heads. so result will 6 dictionaries for each layer
