...
class PrefixTuning(PretrainedBartModel):
    ...
    def __init__(self, config, preseqlen=5):
        ...
        self.mid_dim = 1024
        self.decod_mid_dim = 512

        self.mid_dim = 1024

        self.preseqlen_decod = 50
        self.preseqlen = 250



        self.input_tokens = torch.arange(self.preseqlen_decod).long()
        self.wte = nn.Embedding(self.preseqlen_decod, self.n_embd)
        self.control_trans = nn.Sequential(
                    nn.Linear(self.n_embd, self.decod_mid_dim),
                    nn.GELU(),
                    nn.Linear(self.decod_mid_dim, self.match_n_layer * 2 * self.n_embd))


        self.wte2 = nn.Embedding(self.preseqlen_decod, self.n_embd)
        self.control_trans2 = nn.Sequential(
                    nn.Linear(self.n_embd, self.decod_mid_dim),
                    nn.GELU(),
                    nn.Linear(self.decod_mid_dim, self.match_n_layer * 2 * self.n_embd))

        self.input_tokens_enc = torch.arange(self.preseqlen).long()
        self.wte_enc = nn.Embedding(self.preseqlen, self.n_embd)
        self.control_trans_enc = nn.Sequential(
            nn.Linear(self.n_embd, self.mid_dim),
            nn.GELU(),
            nn.Linear(self.mid_dim, 1024),
            nn.GELU(),
            nn.Linear(1024, int(self.match_n_layer/2) * 2 * self.n_embd))
    ...
    def get_prompt(self, control_code=None, gpt2=None, bsz=None, sample_size=1):
        ...
        input_tokens = self.input_tokens.unsqueeze(0).expand(bsz, -1).to(
            self.device)  # 16 x 200 rows of tensor([[  0,   1,   2,  ..., 197, 198, 199],
        temp_control = self.wte(
            input_tokens)  # self.wte = Embedding(200, 768) , so randomized. # temp_control is 16, 200, 768
        past_key_values = self.control_trans(
            temp_control)  # bsz, seqlen, 2*layer*emb  # Sequential( # past key values.shape is 16 x 200 x 9216 (

        bsz, seqlen, final = past_key_values.shape
        past_key_values_pad = torch.zeros(bsz, self.preseqlen - seqlen, final).to(self.device)
        past_key_values = torch.cat([past_key_values_pad, past_key_values], dim = 1)
        ...
        past_key_values = past_key_values.permute([2, 0, 3, 1, 4]).split(
            2)  # splitting into 12 lots of 2 # [2, 16, 12, 200, 64] query/key for each layer
        ...
        past_key_values2 = torch.cat([past_key_values_pad, past_key_values2], dim = 1)
        ...
        input_tokens_enc = self.input_tokens_enc.unsqueeze(0).expand(old_bsz, -1).to(self.device)
        temp_control_enc = self.wte_enc(input_tokens_enc)
        past_key_values_enc = self.control_trans_enc(temp_control_enc)  # bsz, seqlen, layer*emb
        bsz_enc, seqlen, _ = past_key_values_enc.shape

        past_key_values_enc = past_key_values_enc.view(bsz_enc, seqlen, int(self.match_n_layer/2) * 2, self.match_n_head,
                                                       self.match_n_embd)
        past_key_values_enc = past_key_values_enc.permute([2, 0, 3, 1, 4]).split(2)

        past_key_values_enc = list(past_key_values_enc)
        Z = torch.zeros(past_key_values_enc[0].shape).to(self.device)
        result = [None] * (len(past_key_values_enc)*2)
        result[::2] = past_key_values_enc
        result[1::2] = [Z for i in range(len(past_key_values_enc))]
        past_key_values_enc = result
        ...
        for i, key_val in enumerate(past_key_values):
            temp_dict = {'self': {"prev_key": key_val[0].contiguous(),
                                  # [16, 12, 200, 64] # for each head there are 12 heads. so result will 6 dictionaries for each layer
