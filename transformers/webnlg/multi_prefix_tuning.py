...
class PrefixTuning(PretrainedBartModel):
    ...
    def __init__(self, config, preseqlen=5):
        ...
        print('m_prefix_len', self.m_prefix_len)
        print('m_prefix_mid_dim', self.m_prefix_mid_dim)
        ...
        for i,j in enumerate(self.categories):
            self.M_Prefixes[f'control_trans_single_{j}'] = nn.Sequential(
                nn.Linear(self.n_embd, self.m_prefix_mid_dim),
                nn.Tanh(),
                nn.Linear(self.m_prefix_mid_dim, self.match_n_layer * 2 * self.n_embd))

            self.M_Prefixes[f'control_trans_enc_single_{j}'] = nn.Sequential(
                nn.Linear(self.n_embd, self.m_prefix_mid_dim),
                nn.Tanh(),
                nn.Linear(self.m_prefix_mid_dim, self.match_n_layer * 2 * self.n_embd))
            self.M_Prefixes[f'control_trans_2_single_{j}'] = nn.Sequential(
                nn.Linear(self.n_embd, self.m_prefix_mid_dim),
                nn.Tanh(),
                nn.Linear(self.m_prefix_mid_dim, self.match_n_layer * 2 * self.n_embd))
        ...
        if self.same_initialization:
            print('SAME INITIALIZATION')
            self.initialize_same(self.M_Prefixes[f'wte_{j}'])
            self.initialize_same(self.M_Prefixes[f'wte_enc_{j}'])
            self.initialize_same(self.M_Prefixes[f'wte_2_{j}'])
    ...
    def get_prompt_multiple_prefix(self, conditional_info,bsz=None, sample_size=1):
        ...
        for category_idx,category in enumerate(self.categories):
            ...
            # Probably should arrange in batch with same CEFR, and for inference use this. Will see how slow it is

            temp_control = self.M_Prefixes[f'wte_{category}'](
                self.seed_multiple)  # self.wte = Embedding(200, 768) ,  # temp_control is 8, 200, 1024. so 8 repeats of the embedding matrix

            past_key_values_multiple = self.M_Prefixes[f'control_trans_single_{category}'](temp_control)  # bsz 3*seqlen ,
            ...
            past_key_values_multiple_2 = self.M_Prefixes[f'control_trans_2_single_{category}'](temp_control_2)
            ...
            temp_control_3 = self.M_Prefixes[f'wte_enc_{category}'](
                self.seed_multiple)  # self.wte = Embedding(200, 768) ,  # temp_control is 8, 200, 1024. so 8 repeats of the embedding matrix
            past_key_values_multiple_3 = self.M_Prefixes[f'control_trans_enc_single_{category}'](temp_control_3)
        ...
        past_key_values = past_key_values.permute([2, 0, 3, 1, 4]).split(
            2)  # splitting into 6 lots of 2 # [2, 16, 12, 200, 64] query/key for each layer
