...
class PrefixTransformer(pl.LightningModule):
    def __init__(
        ...
        **config_kwargs
        ...
        if tokenizer is None:
            ...
            if self.hparams.new_tokens:
                ...
                if self.hparams.control_token_DART:
                    new_tokens.extend(['<e2e>', '<webnlg>', '<WikiTableQuestions_lily>', '<WikiSQL_decl_sents>',
                                   '<WikiTableQuestions_mturk>', '<WikiSQL_lily>'])
        ...
        if self.hparams.MULTIPLE_PREFIX:
        ...
        if self.hparams.MULTIPLE_PREFIX:
            ...
            config_prefix.same_intialization = self.hparams.same_intialization
        if self.hparams.unseen:
            config_prefix.unseen = True

        # print(config_prefix)
        if self.hparams.MULTIPLE_PREFIX:
            from prefix_tuning3 import PrefixTuning
        else:
        ...
        if self.hparams.prefixModel_name_or_path is not None:
            ...
            config_prefix.same_initialization = False
            # from utils2 import pickle_save
            # pickle_save(config_prefix,'config_prefix.pkl')
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--same_intialization", type = bool, default = False)
...
def add_generic_args(parser, root_dir) -> None:
    ...
    parser.add_argument("--control_token_DART", type=bool, default=False, help="random seed for initialization")
